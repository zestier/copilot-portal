				...
				{#if request.allowFreeform}
					...
					>
						<input
							...
							disabled={busy}
						/>
						<button type="submit" class="btn primary" disabled={busy || !userInputAnswer}
							>Send</button
						>
					</form>
				{/if}
							...
							<label class="field">
								...
								{:else if field.type === 'string'}
									<input
										...
										value={String(formValues[name] ?? '')}
										oninput={(e) =>
											(formValues[name] = (e.currentTarget as HTMLInputElement).value)}
									/>
		...
		{:else if request.kind === 'exit_plan_mode'}
			...
			<div class="actions">
				<button
					class="btn"
					disabled={busy}
					onclick={() => pick({ kind: 'exit_plan_mode', approved: false })}
		...
		{:else if request.kind === 'sampling'}
			...
			<div class="actions">
				<button
					class="btn"
					disabled={busy}
					onclick={() => pick({ kind: 'sampling', action: 'ack' })}>Dismiss</button
				>
			</div>
		{:else if request.kind === 'mcp_oauth'}
			...
			<div class="actions">
				<button
					class="btn"
					disabled={busy}
					onclick={() => pick({ kind: 'mcp_oauth', action: 'ack' })}>Dismiss</button
				>
			</div>
